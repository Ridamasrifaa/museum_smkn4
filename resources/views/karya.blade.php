...
@push('styles')
    <style>
        ...
        .dark .badge-custom {
            box-shadow: 2px 2px 0px 0px #f4f4f5;
        }
...
@section('content')
        ...
        <div class="mx-auto max-w-7xl border-2 border-zinc-900 dark:border-zinc-100 rounded-3xl bg-sky-400 dark:bg-sky-900 relative overflow-hidden shadow-[4px_4px_0px_0px_#18181b] dark:shadow-[4px_4px_0px_0px_#f4f4f5]">
            ...
            <div class="relative z-10 px-6 py-8 sm:p-10 lg:p-12 flex flex-col md:flex-row items-center justify-between gap-6 text-zinc-900 dark:text-white">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    <div class="bg-white dark:bg-zinc-900 border-2 border-zinc-900 dark:border-zinc-100 px-4 py-3 rounded-2xl shadow-[3px_3px_0px_0px_#18181b] dark:shadow-[3px_3px_0px_0px_#f4f4f5] text-center">
    ...
    <section id="karya" class="py-12 flex-grow bg-grid-pattern">
        <div class="mx-auto max-w-7xl px-4 lg:px-8">
            ...
            <div class="bg-white dark:bg-zinc-900 border-2 border-zinc-900 dark:border-zinc-100 rounded-2xl p-4 mb-6 shadow-[4px_4px_0px_0px_#18181b] dark:shadow-[4px_4px_0px_0px_#f4f4f5]">
                <div class="flex flex-col lg:flex-row items-center gap-4">
                    ...
                    <div class="flex flex-wrap gap-2 w-full lg:w-auto">
                        ...
                        <button data-filter="pplg" class="filter-pill px-4 py-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 cursor-pointer transition">PPLG</button>
                        <button data-filter="dkv" class="filter-pill px-4 py-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 cursor-pointer transition">DKV</button>
                        <button data-filter="toi" class="filter-pill px-4 py-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 cursor-pointer transition">TOI</button>
                        <button data-filter="tkj" class="filter-pill px-4 py-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 cursor-pointer transition">TKJ</button>
                        <button data-filter="tsm" class="filter-pill px-4 py-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 cursor-pointer transition">TSM</button>
                    </div>
                    ...
                    <div class="relative flex-grow w-full">
                        ...
                        <input id="searchInput" type="text" placeholder="Cari judul karya atau nama siswa..."
                            class="w-full pl-10 pr-4 py-2.5 rounded-xl border-2 border-zinc-900 dark:border-zinc-100 bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white placeholder-zinc-500 font-bold text-xs focus:outline-none shadow-[2px_2px_0px_#18181b] dark:shadow-[2px_2px_0px_#f4f4f5]" />
                    </div>
            ...
            <div id="allKaryaGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @forelse($karyas as $karya)
                    <div class="karya-card bg-white dark:bg-zinc-900 rounded-3xl overflow-hidden border-2 border-zinc-900 dark:border-zinc-100 shadow-[6px_6px_0px_0px_#18181b] dark:shadow-[6px_6px_0px_0px_#f4f4f5] card-hover transition-all flex flex-col"
                        ...
                        data-avatar="{{ isset($karya->user->avatar) && $karya->user->avatar ? asset('storage/' . $karya->user->avatar) : '' }}"
                        ...
                        <div class="iframe-container border-b-2 border-zinc-900 dark:border-zinc-100 cursor-pointer" onclick="openModal(this.closest('.karya-card'))">
                            @if ($karya->file_path)
            ...
            <div id="paginationContainer" class="mt-10 flex justify-center items-center w-full"></div>
        </div>
    </section>
    ...
    <div id="detailModal" class="hidden fixed inset-0 bg-zinc-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-zinc-900 rounded-3xl shadow-[8px_8px_0px_0px_#18181b] dark:shadow-[8px_8px_0px_0px_#f4f4f5] max-w-xl w-full p-6 border-2 border-zinc-900 dark:border-zinc-100 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center pb-4 border-b-2 border-zinc-200 dark:border-zinc-700">
                ...
                <button onclick="closeModal()" class="px-3 py-1 bg-yellow-300 hover:bg-yellow-400 text-zinc-900 rounded-xl border-2 border-zinc-900 font-black cursor-pointer shadow-[2px_2px_0px_#000]">✕</button>
            </div>
            ...
            <div class="py-4 space-y-4">
                ...
                <div class="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl border-2 border-zinc-900 dark:border-zinc-100 shadow-[3px_3px_0px_0px_#18181b] dark:shadow-[3px_3px_0px_0px_#f4f4f5]">
                    <div class="flex items-center gap-3">
                        <div id="modalAvatar" class="w-10 h-10 bg-yellow-300 text-zinc-900 border-2 border-zinc-900 rounded-xl flex items-center justify-center font-black text-sm shrink-0 shadow-[2px_2px_0px_#000] overflow-hidden"></div>
                        <div>
                            <div id="modalSiswaContainer"></div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        window.openModal = function (card) {
            ...
            if (actionContainer) {
                ...
                if (d.live && d.live.trim() !== "") {
                    hasButton = true;
                    actionContainer.innerHTML += `<a href="${d.live}" target="_blank" rel="noopener noreferrer" class="flex-1 py-2.5 bg-yellow-300 hover:bg-yellow-400 text-zinc-900 font-black text-xs rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_#000] text-center transition">Live Demo</a>`;
                }
                if (d.github && d.github.trim() !== "") {
                    hasButton = true;
                    const isPplg = (d.category || "").trim().toUpperCase() === "PPLG";
                    const label = isPplg ? "GitHub" : "Link Lainnya";
                    const icon = isPplg ? GITHUB_ICON_SVG : "";
                    actionContainer.innerHTML += `<a href="${d.github}" target="_blank" rel="noopener noreferrer" class="flex-1 py-2.5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 shadow-[2px_2px_0px_#000] text-center transition inline-flex items-center justify-center">${icon}${label}</a>`;
                }
            ...
            if (modalAvatar) {
                if (d.avatar && d.avatar.trim() !== "") {
                    modalAvatar.innerHTML = `<img src="${d.avatar}" alt="${d.siswa}" class="w-full h-full object-cover" />`;
                } else {
            ...
            if (modalGuru) {
                modalGuru.textContent = d.guru && d.guru !== "-" ? "Guru: " + d.guru : "";
            }
        ...
        function runFilter() {
            ...
            filteredCards = allCards.filter((card) => {
                const d = card.dataset;
                const haystack = `${d.title} ${d.siswa} ${d.tech} ${d.category} ${d.desc}`.toLowerCase();
        ...
        function renderPagination(totalPages) {
            ...
            if (currentPage > 1) {
                ...
                prevBtn.textContent = "‹";
                prevBtn.className = "w-8 h-8 rounded-lg text-xs font-black border-2 border-zinc-900 dark:border-zinc-100 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white cursor-pointer hover:bg-yellow-300 transition flex items-center justify-center";
                prevBtn.onclick = () => { currentPage--; renderPage(); scrollToTop(); };
            ...
            for (let i = startPage; i <= endPage; i++) {
                ...
                pageBtn.className = `w-8 h-8 rounded-lg text-xs font-black border-2 border-zinc-900 dark:border-zinc-100 transition cursor-pointer flex items-center justify-center ${
                    isActive 
                        ? 'bg-yellow-300 text-zinc-900 shadow-[2px_2px_0px_#18181b] dark:shadow-[2px_2px_0px_#f4f4f5]' 
                        : 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white hover:bg-zinc-100 dark:hover:bg-zinc-700'
                }`;
                pageBtn.onclick = () => { currentPage = i; renderPage(); scrollToTop(); };
            ...
            if (currentPage < totalPages) {
                ...
                nextBtn.textContent = "›";
                nextBtn.className = "w-8 h-8 rounded-lg text-xs font-black border-2 border-zinc-900 dark:border-zinc-100 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white cursor-pointer hover:bg-yellow-300 transition flex items-center justify-center";
                nextBtn.onclick = () => { currentPage++; renderPage(); scrollToTop(); };
        ...
        function scrollToTop() {
            ...
            if (karyaSection) {
                karyaSection.scrollIntoView({ behavior: 'smooth' });
            }
        }
        ...
        filterPills.forEach((pill) => {
            pill.addEventListener("click", function () {
                filterPills.forEach((p) => {
                    p.classList.remove("bg-yellow-300", "text-zinc-900", "shadow-[2px_2px_0px_#000]");
                    p.classList.add("bg-white", "dark:bg-zinc-800", "text-zinc-900", "dark:text-white");
                });

                this.classList.remove("bg-white", "dark:bg-zinc-800", "text-zinc-900", "dark:text-white");
                this.classList.add("bg-yellow-300", "text-zinc-900", "shadow-[2px_2px_0px_#000]");
